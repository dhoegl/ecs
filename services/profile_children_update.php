<?php
// ***************************************************************
// Process profile update - CHILDREN (name, gender, birthdate, etc.) INFO: Called from profile.php
...
// *********** Submit Child Update to Directory and Children tables **************/
    case isset($_POST['submit1children']):
	    ...
	    $childupdate = $mysql->query($childupdatequery) or die("A database error occurred when trying to update child info. See profile_children_update.php. Error:" . $mysql->errno . " : " . $mysql->error);
        ...
        $childtablecheck = $mysql->query($childtablecheckquery) or die("A database error occurred when trying to check whether child exists in childtable. See profile_children_update.php. Error #: " . $mysql->errno . " : " . $mysql->error);
        ...
        // Add child table entry ID to Directory Table
            ...
            $childtableupdate = $mysql->query($childtableupdatequery) or die("A database error occurred when trying to update child ID in Directory table. See profile_children_update.php. Error:" . $mysql->errno . " : " . $mysql->error);
        ...
        else
        {
            ...
            $childtableupdate = $mysql->query($childtableupdatequery) or die("A database error occurred when trying to update child info. See profile_children_update.php. Error : " . $mysql->errno . " : " . $mysql->error);
            $chronvar = "1";
            $childUpdate_IDquery = "SELECT * FROM " . $_SESSION['childtablename'] . " WHERE child_entry_chron = " . $chronvar;
            $childUpdate_ID = $mysql->query($childUpdate_IDquery) or die("A database error occurred when trying to get child ID when updating children table. See profile_children_update.php. Error:" . $mysql->errno . " : " . $mysql->error);
